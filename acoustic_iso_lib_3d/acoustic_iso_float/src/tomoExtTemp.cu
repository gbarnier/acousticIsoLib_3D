...
// 	// Reset the time slices to zero
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		if (its < host_nts-2){
			...
			// cuda_call(cudaMemcpyAsync(pin_wavefieldSlice1[iGpu], wavefield1+(its+2)*host_nVel, host_nVel*sizeof(float), cudaMemcpyHostToHost, transferStreamIn));
		...
		dev_pDtTemp[iGpu] = NULL;
  		cuda_call(cudaMemsetAsync(dev_pDt2[iGpu], 0, host_nVel*sizeof(float), compStreamIn)); // Reinitialize dev_pRight to zero (because of the += in the kernel)
...
// 		if (its < host_nts-2){
// 			// Copy wavefield slice its+2 from RAM > dev_pStream
// 			std::memcpy(pin_wavefieldSlice1[iGpu], wavefield2+(its+2)*host_nVel, host_nVel*sizeof(float)); // -> this should be done with transfer stream
// 			// cuda_call(cudaMemcpyAsync(pin_wavefieldSlice[iGpu], wavefield2+(its+2)*host_nVel, host_nVel*sizeof(float), cudaMemcpyHostToHost, transferStreamH2DIn));
// 			cuda_call(cudaStreamSynchronize(compStreamIn));
// 			cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu], host_nVel*sizeof(float), cudaMemcpyHostToDevice, transferStreamH2DIn));
// 		}
//
...
// 		dev_pRight[iGpu] = dev_pTemp[iGpu];
// 		dev_pTemp[iGpu] = NULL;
// 		cuda_call(cudaMemsetAsync(dev_pRight[iGpu], 0, host_nModel*sizeof(float), compStreamIn));
//
// 		// Wait until the transfer from pinned -> pStream is completed
// 		cuda_call(cudaStreamSynchronize(transferStreamH2DIn));
// 	}
...
// Source -> reflectivity -> model <- data
void computeTomoLeg1HxHyAdj_3D(float *dev_modelTomoIn, float *wavefield1, float *wavefield2, float *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, int nReceiversRegIn, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, int nBlockDataIn, float *dev_dataRegDtsQcIn){
...
// Leg 2 tomo adjoint
// Source -> reflectivity -> model <- data
void computeTomoLeg2HxHyAdj_3D(float *dev_modelTomoIn, float *wavefield1, float *wavefield2, float *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, int nReceiversRegIn, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, int nBlockDataIn, float *dev_dataRegDtsQcIn){
...
void computeTomoLeg2HxHyFwd_3D(float *dev_modelTomoIn, float *wavefield1, float *wavefield2, float *dev_dataRegDtsIn, float *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, int nBlockDataIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, cudaStream_t transferStreamD2HIn){
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		// Copy wavefield value at its+1 from pStream -> pSourceWavefield
		std::memcpy(pin_wavefieldSlice1[iGpu], wavefield1+(its+1)*host_nVel, host_nVel*sizeof(float));
		cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu], host_nVel*sizeof(float), cudaMemcpyHostToDevice));
		cuda_call(cudaMemcpy(dev_pSourceWavefield[iGpu], dev_pStream[iGpu], host_nVel*sizeof(float), cudaMemcpyDeviceToDevice));
		...
		cuda_call(cudaMemcpy(dev_pDt0[iGpu], dev_pDt1[iGpu], host_nVel*sizeof(float), cudaMemcpyDeviceToDevice));
		cuda_call(cudaMemcpy(pin_wavefieldSlice2[iGpu], dev_pDt0[iGpu], host_nVel*sizeof(float), cudaMemcpyDeviceToHost));
		std::memcpy(wavefield2+its*host_nVel, pin_wavefieldSlice2[iGpu], host_nVel*sizeof(float));
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		// Copy wavefield value at its+1 from pStream -> pSourceWavefield
		std::memcpy(pin_wavefieldSlice1[iGpu], wavefield2+(its+1)*host_nVel, host_nVel*sizeof(float));
		cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu], host_nVel*sizeof(float), cudaMemcpyHostToDevice));
		cuda_call(cudaMemcpyAsync(dev_pSourceWavefield[iGpu], dev_pStream[iGpu], host_nVel*sizeof(float), cudaMemcpyDeviceToDevice, compStreamIn));
