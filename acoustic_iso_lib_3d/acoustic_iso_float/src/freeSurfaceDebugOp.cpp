...
freeSurfaceDebugOp::freeSurfaceDebugOp(std::shared_ptr<SEP::float3DReg> vel, std::shared_ptr<paramObj> par, int nGpu, int iGpu, int iGpuId, int iGpuAlloc){
	...
	// 	}
	// }
	std::shared_ptr<float3DReg> dimTemp;
	...
	dimTemp = std::make_shared<float3DReg>(dimTempHyper);
	setDomainRange(dimTemp, dimTemp);

}

bool freeSurfaceDebugOp::checkParfileConsistency_3D(const std::shared_ptr<SEP::float3DReg> model, const std::shared_ptr<SEP::float3DReg> data) const{
...
void freeSurfaceDebugOp::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {

	if (!add) data->scale(0.0);

	std::shared_ptr<float3DReg> dataTemp, modelTemp;
	...
	modelTemp = std::make_shared<float3DReg>(hyperTemp);
	dataTemp = std::make_shared<float3DReg>(hyperTemp);
	modelTemp->scale(0.0);
	dataTemp->scale(0.0);
...
void freeSurfaceDebugOp::adjoint(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {
