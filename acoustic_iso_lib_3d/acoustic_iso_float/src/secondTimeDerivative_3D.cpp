...
secondTimeDerivative_3D::secondTimeDerivative_3D(int nt, float dt) {
	_nt = nt;
	_dt2 = 1.0 / (dt*dt);
}

void secondTimeDerivative_3D::forward(const bool add, const std::shared_ptr<float2DReg> model, std::shared_ptr<float2DReg> data) const
{
...
void secondTimeDerivative_3D::adjoint(const bool add, std::shared_ptr<float2DReg> model, const std::shared_ptr<float2DReg> data) const{
