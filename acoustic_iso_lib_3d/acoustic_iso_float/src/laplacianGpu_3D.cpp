...
laplacianGpu_3D::laplacianGpu_3D(std::shared_ptr<SEP::float3DReg> vel, std::shared_ptr<paramObj> par, int nGpu, int iGpu, int iGpuId, int iGpuAlloc){
...
void laplacianGpu_3D::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {

	if (!add) data->scale(0.0);

	// std::cout << "Laplacian forward... " << std::endl;
    std::shared_ptr<float3DReg> dataTemp;
    dataTemp=data->clone();
	std::shared_ptr<float3DReg> modelTemp;
...
void laplacianGpu_3D::adjoint(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {
