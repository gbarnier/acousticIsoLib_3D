...
template <class V1, class V2>
void seismicOperator_3D <V1, V2>::setSources_3D(std::shared_ptr<deviceGpu_3D> sourcesDevices, std::shared_ptr<float2DReg> sourcesSignals){
	...
	_sourcesSignalsRegDts = std::make_shared<float2DReg>(_fdParam_3D->_nts, _nSourcesReg); // Source signal interpolated to the regular grid
	_sourcesSignalsRegDtsDt2 = std::make_shared<float2DReg>(_fdParam_3D->_nts, _nSourcesReg); // Source signal with second-order time derivative
	_sourcesSignalsRegDtwDt2 = std::make_shared<float2DReg>(_fdParam_3D->_ntw, _nSourcesReg); // Source signal with second-order time derivative on fine time-sampling grid
	_sourcesSignalsRegDtw = std::make_shared<float2DReg>(_fdParam_3D->_ntw, _nSourcesReg); // Source signal on fine time-sampling grid
...
template <class V1, class V2>
void seismicOperator_3D <V1, V2>::setAcquisition_3D(std::shared_ptr<deviceGpu_3D> sources, std::shared_ptr<float2DReg> sourcesSignals, std::shared_ptr<deviceGpu_3D> receivers, const std::shared_ptr<V1> model, const std::shared_ptr<V2> data){
...
void seismicOperator_3D <V1, V2>::scaleSeismicSource_3D(const std::shared_ptr<deviceGpu_3D> seismicSource, std::shared_ptr<V2> signal) const{
	...
	if (_ginsu == 0){
		#pragma omp parallel for
		for (int iGridPoint = 0; iGridPoint < seismicSource->getNDeviceReg(); iGridPoint++){
			float scale = _fdParam_3D->_vel2Dtw2[pos[iGridPoint]];
			for (int it = 0; it < signal->getHyper()->getAxis(1).n; it++){
				(*sig)[iGridPoint][it] = (*sig)[iGridPoint][it] * scale;
			}
		}
		...
		#pragma omp parallel for
		for (int iGridPoint = 0; iGridPoint < seismicSource->getNDeviceReg(); iGridPoint++){
			float scale = _fdParam_3D->_vel2Dtw2Ginsu[pos[iGridPoint]];
			for (int it = 0; it < signal->getHyper()->getAxis(1).n; it++){
				(*sig)[iGridPoint][it] = (*sig)[iGridPoint][it] * scale;
			}
		}
