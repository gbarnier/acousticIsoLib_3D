...
class deviceGpu_3D : public Operator<SEP::float2DReg, SEP::float2DReg> {
	...
	public:
		...
		// Other functions
  		void checkOutOfBounds(const std::shared_ptr<float1DReg> zCoord, const std::shared_ptr<float1DReg> xCoord, const std::shared_ptr<float1DReg> yCoord, const std::shared_ptr<float3DReg> vel); // For constructor #1
		void checkOutOfBounds(const int &nzDevice, const int &ozDevice, const int &dzDevice , const int &nxDevice, const int &oxDevice, const int &dxDevice, const int &nyDevice, const int &oyDevice, const int &dyDevice, const std::shared_ptr<float3DReg> vel); // For constructor #3
