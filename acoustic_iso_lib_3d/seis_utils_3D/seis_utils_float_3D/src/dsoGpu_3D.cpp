...
dsoGpu_3D::dsoGpu_3D(int nz, int nx, int ny, int nExt1, int nExt2, int fat, double zeroShift){
...
void dsoGpu_3D::forward(const bool add, const std::shared_ptr<double5DReg> model, std::shared_ptr<double5DReg> data) const {

	if (!add) data->scale(0.0);

    for (int iExt2=0; iExt2<_nExt2; iExt2++){
        double weight2 = 1.0*(std::abs(iExt2-_hExt2)); // Compute distance from physical axis #2
        for (int iExt1=0; iExt1<_nExt1; iExt1++){
            double weight1 = 1.0*(std::abs(iExt1-_hExt1)); // Compute distance from physical axis #1
...
void dsoGpu_3D::adjoint(const bool add, std::shared_ptr<double5DReg> model, const std::shared_ptr<double5DReg> data) const {
