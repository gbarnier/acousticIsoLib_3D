...
void allocateSetTomoExtGinsuGpu_3D(int nz, int nx, int ny, int minPad, int blockSize, double alphaCos, double *vel2Dtw2, double *reflectivityScale, double *extReflectivity, int iGpu, int iGpuId){
    ...
    // Reflectivity model
    cuda_call(cudaMalloc((void**) &dev_modelTomo[iGpu], host_nVel_ginsu[iGpu]*sizeof(double)));
...
void tomoTauShotsFwdGinsuGpu_3D(double *model, double *dataRegDts, double *extReflectivity, double *sourcesSignals, long long *sourcesPositionReg, int nSourcesReg, long long *receiversPositionReg, int nReceiversReg, int slowSquare, int iGpu, int iGpuId){
	...
	double *dummySliceRight,*dummyRef;
	dummySliceRight = new double[host_nVel_ginsu[iGpu]];
	dummyRef = new double[host_nModelExt_ginsu[iGpu]];
	...
	/**************************************************************************/
	// Copy model to device
	cuda_call(cudaMemcpy(dev_modelTomo[iGpu], model, host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
	...
	//////////////////////////////// Debug /////////////////////////////////
    // cuda_call(cudaMemcpy(dummySliceRight, dev_reflectivityScale[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost));
	...
	//////////////////////////////// Debug /////////////////////////////////
    // cuda_call(cudaMemcpy(dummyRef, dev_extReflectivity[iGpu], host_nModelExt_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost));
...
void tomoTauShotsAdjGinsuGpu_3D(double *model, double *dataRegDts, double *extReflectivity, double *sourcesSignals, long long *sourcesPositionReg, int nSourcesReg, long long *receiversPositionReg, int nReceiversReg, int slowSquare, int iGpu, int iGpuId){
	...
	// Copy model to host
	cuda_call(cudaMemcpy(model, dev_modelTomo[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost));
