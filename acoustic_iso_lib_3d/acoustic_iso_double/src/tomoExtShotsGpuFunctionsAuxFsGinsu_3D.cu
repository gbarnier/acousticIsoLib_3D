...
void computeTomoSrcWfldDt2FsGinsu_3D(double *dev_sourcesIn, long long *dev_sourcesPositionsRegIn, int nSourcesRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamIn){
	...
	cuda_call(cudaMemset(dev_pDt0[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double)));
  	cuda_call(cudaMemset(dev_pDt1[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double)));
	cuda_call(cudaMemset(dev_pDt2[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double)));
	cuda_call(cudaMemset(dev_pSourceWavefield[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double)));
	...
	for (int it2 = 1; it2 < host_sub+1; it2++){
        ...
        setFreeSurfaceConditionFwdGinsuGpu_3D<<<dimGridFreeSurfaceIn, dimBlockFreeSurfaceIn, 0, compStreamIn>>>(dev_p1[iGpu], iGpu);
		...
		// Damp wavefields
		dampCosineEdgeFreeSurfaceGinsu_32_3D<<<dimGrid32In, dimBlock32In, 0, compStreamIn>>>(dev_p0[iGpu], dev_p1[iGpu], iGpu);
	...
	// Copy pDt1 (its = 0)
	cuda_call(cudaMemcpyAsync(dev_pDt1[iGpu], dev_pLeft[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
	...
	dev_pTemp[iGpu] = NULL;
	cuda_call(cudaMemsetAsync(dev_pRight[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double), compStreamIn));
	...
	for (int its = 1; its < host_nts-1; its++){
		...
		cuda_call(cudaMemcpyAsync(dev_pDt2[iGpu], dev_pLeft[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
		...
		cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], dev_pSourceWavefield[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
		...
		cuda_call(cudaMemcpyAsync(pin_wavefieldSlice1[iGpu]+(its-1)*host_nVel_ginsu[iGpu], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost, transferStreamIn));
	...
	cuda_call(cudaMemcpyAsync(pin_wavefieldSlice1[iGpu]+(host_nts-2)*host_nVel_ginsu[iGpu], dev_pSourceWavefield[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost, compStreamIn));

	// Reset pDt0 and compute second-order time-derivative at nts-1
	cuda_call(cudaMemsetAsync(dev_pDt0[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double), compStreamIn));
	...
	cuda_call(cudaMemcpyAsync(pin_wavefieldSlice1[iGpu]+(host_nts-1)*host_nVel_ginsu[iGpu], dev_pSourceWavefield[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost, compStreamIn));

}

// Receiver wavefield
void computeTomoRecWfldFsGinsu_3D(double *dev_dataRegDtsIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, int nBlockDataIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamIn){
	...
	for (int its = host_nts-2; its > -1; its--){
		...
		for (int it2 = host_sub-1; it2 > -1; it2--){
            ...
            // Step adjoint
			stepAdjFreeSurfaceGinsuGpu_3D<<<dimGridIn, dimBlockIn, 0, compStreamIn>>>(dev_p0[iGpu], dev_p1[iGpu], dev_p0[iGpu], dev_vel2Dtw2[iGpu], iGpu);
		...
		cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], dev_pRight[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));

		// Wait until pStream has been updated
		cuda_call(cudaStreamSynchronize(compStreamIn));

		// Transfer pStream -> pin (at its+1)
		cuda_call(cudaMemcpyAsync(pin_wavefieldSlice2[iGpu]+(its+1)*host_nVel_ginsu[iGpu], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost, transferStreamIn));
		...
		dev_pTemp[iGpu] = NULL;
		cuda_call(cudaMemsetAsync(dev_pLeft[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double), compStreamIn));
	...
	cuda_call(cudaMemcpy(pin_wavefieldSlice2[iGpu], dev_pRight[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost));

}

/************************** Forward - Time-lags *******************************/
// Source -> reflectivity -> model -> data
void computeTomoLeg1TauFwdFsGinsu_3D(double *dev_modelTomoIn, double *dev_dataRegDtsIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int nBlockDataIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, cudaStream_t transferStreamD2HIn){


	double *dummySliceRight;
	dummySliceRight = new double[host_nVel_ginsu[iGpu]];
	...
	for (int iExt=0; iExt<4*host_hExt1+1; iExt++){
		...
		if (iExt < 2*host_hExt1+1){
			...
			cuda_call(cudaMemcpy(dev_pSourceWavefieldTau[iGpu][iExt], pin_wavefieldSlice1[iGpu]+iExt*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
		}
	}
	...
	// Launch transfer
	cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+(2*host_hExt1+1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
	...
	if (host_hExt1 > 0){
		cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][its+2*host_hExt1+1], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
	} else {
		cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][0], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
	}
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		if (its < 2*host_hExt1-1) {
			...
			// Launch transfer
			cuda_call(cudaStreamSynchronize(compStreamIn));
			cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+(its+2*host_hExt1+2)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
		...
		cuda_call(cudaMemcpyAsync(dev_pDt0[iGpu], dev_pDt1[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));

		// Wait until pDt0 is ready to be transfered
		cuda_call(cudaStreamSynchronize(compStreamIn));

		//////////////////////////////// Debug /////////////////////////////////
		// cuda_call(cudaMemcpy(dummySliceRight, dev_pDt0[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost));
		...
		cuda_call(cudaMemcpyAsync(pin_wavefieldSlice2[iGpu]+its*host_nVel_ginsu[iGpu], dev_pDt0[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost, transferStreamD2HIn));
		...
		dev_pDt2[iGpu] = dev_pDtTemp[iGpu];
		dev_pDtTemp[iGpu] = NULL;
  		cuda_call(cudaMemsetAsync(dev_pDt2[iGpu], 0, host_nVel_ginsu[iGpu]*sizeof(double), compStreamIn));
		...
		if (its < 2*host_hExt1-1) {
			...
			cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][its+2*host_hExt1+2], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
		}
		...
		if (its >= 2*host_hExt1-1 && its < host_nts-2*host_hExt1-2) {
			...
			// Copy pStream -> device
			cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][4*host_hExt1], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
	...
	cuda_call(cudaMemcpyAsync(pin_wavefieldSlice2[iGpu]+(host_nts-1)*host_nVel_ginsu[iGpu], dev_pDt1[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));

	// Wait until pDt1 -> pinned wavefield is done
	cuda_call(cudaStreamSynchronize(compStreamIn));

	//////////////////////////////// Debug /////////////////////////////////
	// cuda_call(cudaMemcpy(dummySliceRight, dev_pRight[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost));
	// std::cout << "its = " << its << std::endl;
	...
	cuda_call(cudaMemcpy(dev_pSourceWavefield[iGpu], pin_wavefieldSlice2[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
	...
	cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));

	// Start propagating scattered wavefield
	for (int its = 0; its < host_nts-1; its++){

		// Copy wavefield value at its+1 from pStream -> pSourceWavefield
		cuda_call(cudaMemcpyAsync(dev_pSourceWavefield[iGpu], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));

		if (its < host_nts-2){
			// Copy wavefield slice its+2 from RAM > dev_pStream
			cuda_call(cudaStreamSynchronize(compStreamIn));
			cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(its+2)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
		}
...
void computeTomoLeg2TauFwdFsGinsu_3D(double *dev_modelTomoIn, double *dev_dataRegDtsIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int nBlockDataIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, cudaStream_t transferStreamD2HIn){
	...
	cuda_call(cudaMemcpy(dev_pSourceWavefield[iGpu], pin_wavefieldSlice1[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));

	// Do first fwd imaging condition for its = 0 (after that, secondary source at its = 0 is done)
	imagingFwdGinsuGpu_3D<<<dimGridIn, dimBlockIn, 0, compStreamIn>>>(dev_modelTomoIn, dev_pLeft[iGpu], dev_pSourceWavefield[iGpu], iGpu);

	// Transfer new slice from pinned -> pStream for time its = 1
	cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
	cuda_call(cudaStreamSynchronize(transferStreamH2DIn));
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		if (its < host_nts-2){
			cuda_call(cudaStreamSynchronize(compStreamIn));
			cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+(its+2)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
		}
	...
	for (int iExt=0; iExt<4*host_hExt1+1; iExt++){
		...
		if (iExt < 2*host_hExt1+1){
			// Transfer from pinned -> GPU
			cuda_call(cudaMemcpy(dev_pSourceWavefieldTau[iGpu][iExt], pin_wavefieldSlice2[iGpu]+iExt*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
		}
	}
	...
	cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(2*host_hExt1+1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		if (its < 2*host_hExt1-1) {
			...
			cuda_call(cudaStreamSynchronize(compStreamIn));
			cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(its+2*host_hExt1+2)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
...
/************************** Forward - Offsets *********************************/
// Source -> reflectivity -> model -> data
void computeTomoLeg1HxHyFwdFsGinsu_3D(double *dev_modelTomoIn, double *dev_dataRegDtsIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int nBlockDataIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, cudaStream_t transferStreamD2HIn){
...
// Source -> model -> reflectivity -> data
void computeTomoLeg2HxHyFwdFsGinsu_3D(double *dev_modelTomoIn, double *dev_dataRegDtsIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int nBlockDataIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, cudaStream_t transferStreamD2HIn){
...
void computeTomoLeg1TauAdjFsGinsu_3D(double *dev_modelTomoIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, int nBlockDataIn){
	...
	cuda_call(cudaMemcpy(dev_pDt0[iGpu], pin_wavefieldSlice2[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
	...
	for (int its = 0; its < host_nts-1; its++){
		...
		cuda_call(cudaMemcpyAsync(dev_pRecWavefield[iGpu], dev_pDt0[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
		...
		cuda_call(cudaMemcpyAsync(dev_pDt0[iGpu], pin_wavefieldSlice2[iGpu]+(its+1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));

		for (int it2 = 1; it2 < host_sub+1; it2++){

            // Apply free surface conditions for Laplacian
			setFreeSurfaceConditionFwdGinsuGpu_3D<<<dimGridFreeSurfaceIn, dimBlockFreeSurfaceIn, 0, compStream[iGpu]>>>(dev_p1[iGpu], iGpu);
			...
			// Spread energy to dev_pDt1 and dev_pDt2
			interpFineToCoarseSliceGinsu_32_3D<<<dimGrid32In, dimBlock32In, 0, compStream[iGpu]>>>(dev_pDt1[iGpu], dev_pDt2[iGpu], dev_p0[iGpu], it2, iGpu);
		...
		/////////////////////////////// QC /////////////////////////////////////
		// std::cout << "its = " << its << std::endl;
		// double *dummySlice, *dummyModel ;
		// dummySlice = new double[host_nVel_ginsu[iGpu]];
		// dummyModel = new double[host_nVel_ginsu[iGpu]];
		// cudaMemcpy(dummySlice, dev_pRecWavefield[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost);
		...
		// cudaMemcpy(dummyModel, dev_pDt1[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToHost);
...
// Source -> model <- reflectivity <- data
void computeTomoLeg2TauAdjFsGinsu_3D(double *dev_modelTomoIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, int nBlockDataIn){
	...
	for (int iExt=4*host_hExt1; iExt>-1; iExt--){
		...
		if (iExt > 2*host_hExt1-1){
			...
			// Transfer from pinned -> GPU
			cuda_call(cudaMemcpy(dev_pSourceWavefieldTau[iGpu][iExt], pin_wavefieldSlice2[iGpu]+(host_nts-1+iExt-4*host_hExt1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
		}
	}
	...
	cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(host_nts-2-2*host_hExt1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
	...
	if (host_hExt1 > 0){
		cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][2*host_hExt1-1], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
	} else {
		cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][0], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
	}
	...
	for (int its = host_nts-2; its > -1; its--){
		...
		if (its > host_nts-2*host_hExt1-1){
			...
			// cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(its-2*host_hExt1-1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
			cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(its-2*host_hExt1-1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
		...
		// Middle part of adjoint propagation
			...
			// Copy slice its-2*host_hExt-1 from pinned -> device
			// cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
			cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice2[iGpu]+(its-2*host_hExt1-1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
		...
		cuda_call(cudaMemcpy(dev_pSourceWavefield[iGpu], pin_wavefieldSlice1[iGpu]+(its+1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
		// cuda_call(cudaMemcpy(dev_pSourceWavefield[iGpu], dev_pDt0[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice));
		...
		if (its > host_nts-2*host_hExt1-1) {
			...
			// cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][iSlice], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
			cuda_call(cudaMemcpy(dev_pSourceWavefieldTau[iGpu][iSlice], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice));
		...
		// Middle part of adjoint propagation
			...
			// Copy pStream -> device
			// cuda_call(cudaMemcpyAsync(dev_pSourceWavefieldTau[iGpu][0], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice, compStreamIn));
			cuda_call(cudaMemcpy(dev_pSourceWavefieldTau[iGpu][0], dev_pStream[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyDeviceToDevice));
		}
...
/************************* Adjoint - Offsets **********************************/
// Source -> reflectivity -> model <- data
void computeTomoLeg1HxHyAdjFsGinsu_3D(double *dev_modelTomoIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, dim3 dimGridFreeSurfaceIn, dim3 dimBlockFreeSurfaceIn, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, int nBlockDataIn){
	...
	cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
...
// Source -> model <- reflectivity <- data
void computeTomoLeg2HxHyAdjFsGinsu_3D(double *dev_modelTomoIn, double *dev_extReflectivityIn, long long *dev_receiversPositionRegIn, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGrid32In, dim3 dimBlock32In, int iGpu, cudaStream_t compStreamIn, cudaStream_t transferStreamH2DIn, int nBlockDataIn){
	...
	cuda_call(cudaMemcpy(dev_pRecWavefield[iGpu], pin_wavefieldSlice2[iGpu]+(host_nts-1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
	...
	cuda_call(cudaMemcpy(dev_pDt0[iGpu], pin_wavefieldSlice2[iGpu]+(host_nts-2)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));

	// Copy source wavefield slice from RAM -> pinned for time its = nts-1 -> transfer to pDt0
	cuda_call(cudaMemcpy(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+(host_nts-1)*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice));
	...
	for (int its = host_nts-2; its > -1; its--){
		...
		cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], pin_wavefieldSlice1[iGpu]+its*host_nVel_ginsu[iGpu], host_nVel_ginsu[iGpu]*sizeof(double), cudaMemcpyHostToDevice, transferStreamH2DIn));
